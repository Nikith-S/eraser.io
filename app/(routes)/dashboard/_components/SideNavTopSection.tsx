    ...
    return (
        ...
        <Popover>
            ...
            <PopoverContent className='ml-7 p-4'>
                ...
                <div>
                    {teamList.map((team) => (
                        <h2 key={team._id} className={`p-2 hover:bg-blue-500 hover:text-white rounded-lg mb-1 cursor-pointer ${activeTeam?._id === team._id ? 
                          'bg-blue-500 text-white' : ''}`} onClick={() => setActiveTeam(team)}>
                            {team.teamName}
                        </h2>
                    ))}
                </div>
                ...
                <div>
                    {menu.map((item) => (
                        <h2 key={item.id} className='flex gap-2 items-center p-2 hover:bg-gray-100 rounded-lg cursor-pointer' onClick={() => onMenuClick(item)}>
